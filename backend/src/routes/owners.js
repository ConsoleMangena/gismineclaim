...
router.get('/', authMiddleware, async (req, res) => {
  ...
  try {
    const [countResult, rowsResult] = await Promise.all([
      ...
      pool.query(
        `SELECT id, name, national_id, contact_info, phone, email, address, created_at
...
router.post('/', authMiddleware, async (req, res) => {
  const { name, national_id, contact_info = '', phone = '', email = '', address = '' } = req.body || {}
  if (!name || !national_id) return sendError(res, 400, 'name and national_id are required.')

  try {
    const result = await pool.query(
      `INSERT INTO spatial_data_owner (name, national_id, contact_info, phone, email, address, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())
       RETURNING id, name, national_id, contact_info, phone, email, address, created_at`,
      [name, national_id, contact_info, phone, email, address]
    )
  ...
  try {
    const result = await pool.query(
      `UPDATE spatial_data_owner
       SET name = $1, national_id = $2, contact_info = $3, phone = $4, email = $5, address = $6
       WHERE id = $7
       RETURNING id, name, national_id, contact_info, phone, email, address, created_at`,
      [name, national_id, contact_info, phone, email, address, id]
    )
